...
# -*- coding: utf-8 -*-
"""
Script đẩy User Story/Use Case Documentation lên Confluence
Tạo page mới với nội dung đầy đủ 7 mục
...
def load_confluence_config(config_path: str = None) -> dict:
    """
    Đọc cấu hình Confluence từ file JSON.
    Ưu tiên: CLI/config file > file trong scripts/ > file trong skill/.
...
def extract_function_name(content: str) -> str:
    """Lấy tên chức năng từ content."""
    # Tìm "Tên chức năng:" hoặc "# [Tên]"
...
def extract_epic_name(content: str) -> str:
    """Lấy tên Epic từ content hoặc path."""
...
def create_or_update_confluence_page(
    ...
    Args:
        ...
        labels: Danh sách labels (list of strings)
    ...
    # Labels mặc định
    if labels is None:
        labels = ["user-story", "use-case"]
    
    # Convert labels sang format Confluence
    label_list = [{"name": label} for label in labels]
...
def main():
    parser = argparse.ArgumentParser(
        description="Đẩy User Story/Use Case Documentation lên Confluence",
        epilog="Cấu hình: confluence.config.json (hoặc --config <path>). Xem SKILL.md.",
    )
    parser.add_argument("file", help="Đường dẫn file markdown User Story")
    parser.add_argument(
        "--config", "-c",
        default=None,
    ...
    # Extract function name và epic
    function_name = extract_function_name(md_content)
    epic_name = extract_epic_name(md_content)
    
    # Title
    title = f"User Story - {function_name} - {datetime.now().strftime('%Y-%m-%d')}"
    
    # Labels
    labels = ["user-story", "use-case"]
    if epic_name:
        # Tạo label từ epic name (slug)
        epic_slug = re.sub(r"[^a-zA-Z0-9]+", "-", epic_name.lower())[:20]
        labels.append(epic_slug)
